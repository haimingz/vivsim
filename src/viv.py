...
ROOT_PATH = os.path.dirname(__file__) + "/../output/viv/"  # path to save the output 
...
for t in tqdm(range(TM)):
    ...
    if SAVE and t % SAVE_EVERY == 0 and t > SAVE_AFTER:
        ...
        jnp.save(ROOT_PATH + f"u_{t}.npy", u[:, ::SAVE_DSAMP, ::SAVE_DSAMP].astype(SAVE_FORMAT))
        jnp.save(ROOT_PATH + f"rho_{t}.npy", u[:, ::SAVE_DSAMP, ::SAVE_DSAMP].astype(SAVE_FORMAT))
...
if SAVE:
    jnp.savez(ROOT_PATH + "dynamics.npz", d=d_save, v=v_save, a=a_save, g=g_save)    
    jnp.save(ROOT_PATH + f"f_{t}.npy", f)
