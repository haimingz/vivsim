...
def update_markers_coords_1dof(X_MARKERS, Y_MARKERS, d):
    ...
    x_markers = X_MARKERS + d[-1]
    y_markers = Y_MARKERS + d[0]
    return x_markers, y_markers


def update_markers_coords_2dof(X_MARKERS, Y_MARKERS, X_CENTER, Y_CENTER, d):
    ...
    x_markers = X_CENTER + d[-1] + x_rel * jnp.cos(d[2]) - y_rel * jnp.sin(d[2]) 
    y_markers = Y_CENTER + d[0] + x_rel * jnp.sin(d[2]) + y_rel * jnp.cos(d[2]) 
    
    return x_markers, y_markers


def update_markers_velocity_2dof(x_markers, y_markers, X_CENTER, Y_CENTER, d, v):
    ...
    """
    x_rel = x_markers - X_CENTER - d[-1]
    y_rel = y_markers - Y_CENTER - d[0]
    
    v_markers = jnp.zeros((x_markers.shape[-1], 2))
    v_markers = v_markers.at[:, -1].set(v[0] - v[2] * y_rel)
    v_markers = v_markers.at[:, 0].set(v[1] + v[2] * x_rel)
    return v_markers
...
def multi_direct_forcing(
    ...
    for _ in range(N_ITER):
        ...
        delta_g_markers = get_g_markers_needed(v_markers, u_markers)
        delta_g = spread_g_needed(delta_g_markers, kernels)
