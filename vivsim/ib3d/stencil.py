...
def get_ib_stencil(
    ...
    stencil_x = stencil_ref[:, 0:1] + stencil_offset_x
    stencil_y = stencil_ref[:, 1:2] + stencil_offset_y
    stencil_z = stencil_ref[:, 2:3] + stencil_offset_z

    stencil_weights = (
        kernel(stencil_x - marker_coords[:, 0:1])
        * kernel(stencil_y - marker_coords[:, 1:2])
        * kernel(stencil_z - marker_coords[:, 2:3])
    )
